...
build-agent:
	CGO_ENABLED=1 GOOS=linux GOARCH=amd64 go build -o build/agent/agentServer cmd/agent/main.go
build-proxy:
	CGO_ENABLED=1 GOOS=linux GOARCH=amd64 go build -o build/proxy/proxyServer cmd/proxy/main.go
